...
/* Header for class com_test_recoder_OpenSLActivity */
...
/*
 * Class:     com_test_recoder_OpenSLActivity
 * Method:    record
...
JNIEXPORT jint JNICALL Java_com_test_recoder_MainActivity_record
   ...
   // time_t current_time = time(NULL) * 1000;
    ...
    if(time > timefromj){ //指定时间
        LOGE("Recoder time  is fail ");
        return -1;
    }else{
        LOGE("Recoder time is ok ");
    }
    SLresult re;
...
/*
 * Class:     com_test_recoder_OpenGLActivity
 * Method:    stopRecod
 * Signature: ()I
 */
